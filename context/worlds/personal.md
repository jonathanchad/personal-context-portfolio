# World: Personal

Family, community roles and personal admin. Agents scheduling or
prioritising on Jonathan's behalf must respect these.

## Family

- **Katie** — wife.
- Two sons.
- One of the boys swims with **Rackley Squad**; sessions are a fixed family
  commitment (e.g. Saturday 9:30–10:30, Newmarket/Centenary pool). When
  Katie travels, Grandad covers squad runs.
- **Wedding anniversary: 4 September.** Protect it.
- **Jonathan swims.** Pan Pacific Masters Games, Gold Coast, 6 to 15 Nov
  2026; his swimming is 12 to 14 Nov (50 free, 50 fly, mixed 100 and
  200 free). Training is fixed. See `goals-and-priorities.md` goal 2.
- **Jonathan is Rackley Swim Team's Centenary Race Volunteer Organiser**
  (centenary.race@rackleyswimteam.com.au). Race-day organising for the
  club is a commitment to protect, same as squad sessions — don't book
  over race days without asking.

## Community

- **Ithaca Creek State School P&C — Jonathan is President.** Under the
  Constitution (cl. 16.2, 21.1, 23.4) the President provides leadership,
  is the accountable officer, represents the Association, chairs
  meetings, and with the Treasurer is responsible for proper collection,
  management and disbursement of money. Full text and the ICSS specifics
  live in the `pandc-compliance` skill's reference files; cite them
  rather than paraphrasing. Anything for the P&C — website copy,
  newsletters, meeting notices, decisions — goes through that skill
  before publishing. Contacts: Jodie Painter
  (jodie.painter@icsspandc.com), executive@icsspandc.com; Chris Erbacher
  on the events side.
- **His two big P&C priorities (Sep to Oct 2026):** (1) get the
  icsspandc.com website up (MVP scope from the June meeting: homepage,
  tuck shop, garden, uniform shop, event RSVP; donate page later);
  (2) the time capsule event below.
- **P&C time capsule event: Sunday 25 October 2026, 10am, school hall,
  49 Lugg Street, Bardon.** His big personal priority through October.
  See `goals-and-priorities.md` Tradeoffs. Keep 24 to 25 Oct clear.
  Related: 8 Sep parent conversation evening (bake stall being handed to
  the parent bake-sale group).

## Personal admin (open)

- Allianz claim 6210520873 — a routine watches for the reply.

## Identity

- Personal email: jonathanchad@gmail.com
- Location: Brisbane (Australia/Brisbane time)

## Rules for agents

- Don't book work over squad, race-day or family commitments without
  asking.
- Family details are in this repo because Jonathan asked for them; keep
  them here, not in client-facing material.
- **Never name the sons.** Jonathan deliberately removed their names
  from this repo; refer to them as "his sons" or "one of the boys",
  never by name, in anything an agent writes or generates.
