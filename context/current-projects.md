...
Source of truth for what Jonathan is actively working on. **Update this
...
Client retainers run on time caps. Watch for over-servicing against those
caps — this is a persistent concern the Weekly AAR checks for every week,
not a one-off flag.

---
_Seeded from the live prompts of the Weekly AAR and Donna routines. Needs a
real pass to confirm current status/priority per item, scope, and what
"done" looks like for each — see GETTING-STARTED.md for the interview
format._
