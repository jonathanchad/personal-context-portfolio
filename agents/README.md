...
| 13 | **Nura Fund reply check** (found 17 Sep, captured 25 Sep — not built in any session this repo tracks) | daily 08:00 | live | one Gmail/Superhuman thread (charlotteproject.au) | report only, noteworthy runs only | Watches one grant-application thread (Meliore Foundation, USD $300k) for a reply; silent when nothing's changed |
| 14 | **Monthly billing reconstruction — BTS** (found 17 Sep, captured 25 Sep — not built in any session this repo tracks) | 2nd of month, 08:00 | live | Notion Sessions/Retainers/Donna Log/Run History/Routing, Granola, Otter, Calendar, Gmail, Todoist | Notion Sessions corrections; .xlsx billing schedule via SendUserFile | Reconstructs the month's actual billable time across 6 evidence sources against Jonathan's chronic under-recording; never invoices or contacts a client |
